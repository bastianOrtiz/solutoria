...
if( $_POST ){
    ...
    if( @$_POST['action'] == 'generar_archivo_previred' ){
        ...
        for ($i=0; $i < $total_reg; $i++) { 
            ...
            $percent_cotizac_trabajo_pesado  = rellenar($_POST['percent_cotizac_trabajo_pesado'][$i], 5, "");
            $cotizacion_trabajo_pesado  = rellenar($_POST['cotizacion_trabajo_pesado'][$i], 6, "");
            $codigo_inst_apvi = rellenar($_POST['codigo_inst_apvi'][$i], 3, "");
            $nro_contrato_apvi   = rellenar($_POST['nro_contrato_apvi'][$i],20, "");
            $forma_pago_apvi = rellenar($_POST['forma_pago_apvi'][$i], 1, "");
            $cotizacion_apvi = rellenar($_POST['cotizacion_apvi'][$i], 8, "");
            $cotizacion_depositos_convencidos = rellenar($_POST['cotizacion_depositos_convencidos'][$i], 8, "");
            $cod_inst_autorizada_apvc = rellenar($_POST['cod_inst_autorizada_apvc'][$i], 3, "");
            $nro_contrato_apvc   = rellenar($_POST['nro_contrato_apvc'][$i],20, "");
            $forma_de_pago_apvc  = rellenar($_POST['forma_de_pago_apvc'][$i], 1, "");
            $cotizacion_trabajador_apvc  = rellenar($_POST['cotizacion_trabajador_apvc'][$i], 8, "");
            $cotizacion_empleador_apvc  = rellenar($_POST['cotizacion_empleador_apvc'][$i], 8, "");
            $rut_afiliado_voluntario  = rellenar($_POST['rut_afiliado_voluntario'][$i],11, "");
            $dv_afiliado_voluntario  = rellenar($_POST['dv_afiliado_voluntario'][$i], 1, "");
            $apellido_paterno_af_voluntario   = rellenar($_POST['apellido_paterno_af_voluntario'][$i],30, "");
            $apellido_materno_af_voluntario   = rellenar($_POST['apellido_materno_af_voluntario'][$i],30, "");
            $nombres_af_voluntario   = rellenar($_POST['nombres_af_voluntario'][$i],30, "");
            $codigo_movimiento_de_personal  = rellenar($_POST['codigo_movimiento_de_personal'][$i], 2, "");
            $fecha_desde_movimiento   = rellenar($_POST['fecha_desde_movimiento'][$i],10, "");
            $fecha_hasta_movimiento   = rellenar($_POST['fecha_hasta_movimiento'][$i],10, "");
            $codigo_de_la_afp = rellenar($_POST['codigo_de_la_afp'][$i], 2, "");
            $monto_capitalizacion_voluntaria = rellenar($_POST['monto_capitalizacion_voluntaria'][$i], 8, "");
            $monto_ahorro_voluntario = rellenar($_POST['monto_ahorro_voluntario'][$i], 8, "");
            $numero_de_periodos_de_cotizacion = rellenar($_POST['numero_de_periodos_de_cotizacion'][$i], 2, "");
            $codigo_ex_caja_regimen  = rellenar($_POST['codigo_ex_caja_regimen'][$i], 4, "");
            $tasa_cotizacion_ex_cajas_de_prevision  = rellenar($_POST['tasa_cotizacion_ex_cajas_de_prevision'][$i], 5, "");
            $renta_imponible_ips = rellenar($_POST['renta_imponible_ips'][$i], 8, "");
            $cotizacion_obligatoria_ips  = rellenar($_POST['cotizacion_obligatoria_ips'][$i], 8, "");
            $renta_imponible_desahucio  = rellenar($_POST['renta_imponible_desahucio'][$i], 8, "");
            $codigo_ex_caja_regimen_desahucio = rellenar($_POST['codigo_ex_caja_regimen_desahucio'][$i], 4, "");
            $tasa_cotizac_desahucio_ex_cajas_prevision  = rellenar($_POST['tasa_cotizac_desahucio_ex_cajas_prevision'][$i], 5, "");
            $cotizacion_desahucio = rellenar($_POST['cotizacion_desahucio'][$i], 8, "");
            $cotizacion_fonasa  = rellenar($_POST['cotizacion_fonasa'][$i], 8, "");
            $cotizacion_acc__trabajo_isl = rellenar($_POST['cotizacion_acc__trabajo_isl'][$i], 8, "");
            $bonificacion_ley_15_386 = rellenar($_POST['bonificacion_ley_15_386'][$i], 8, "");
            $descuento_por_cargas_familiares_isl = rellenar($_POST['descuento_por_cargas_familiares_isl'][$i], 8, "");
            $bonos_de_gobierno  = rellenar($_POST['bonos_de_gobierno'][$i], 8, "");
            $codigo_institucion_de_salud = rellenar($_POST['codigo_institucion_de_salud'][$i], 2, "");
            $numero_de_fun   = rellenar($_POST['numero_de_fun'][$i],16, "");
            $renta_imponible_isapre  = rellenar($_POST['renta_imponible_isapre'][$i], 8, "");
            $moneda_del_plan_pactado_isapre  = rellenar($_POST['moneda_del_plan_pactado_isapre'][$i], 1, "");
            $cotizacion_pactada  = rellenar($_POST['cotizacion_pactada'][$i], 8, "");
            $cotizacion_obligatoria_isapre  = rellenar($_POST['cotizacion_obligatoria_isapre'][$i], 8, "");
            $cotizacion_adicional_voluntaria = rellenar($_POST['cotizacion_adicional_voluntaria'][$i], 8, "");
            $monto_ges_futuro = rellenar($_POST['monto_ges_futuro'][$i], 8, "");
            $codigo_ccaf = rellenar($_POST['codigo_ccaf'][$i], 2, "");
            $renta_imponible_ccaf = rellenar($_POST['renta_imponible_ccaf'][$i], 8, "");
            $creditos_personales_ccaf = rellenar($_POST['creditos_personales_ccaf'][$i], 8, "");
            $dscto_dental_ccaf  = rellenar($_POST['dscto_dental_ccaf'][$i], 8, "");
            $dsctos_por_leasing  = rellenar($_POST['dsctos_por_leasing'][$i], 8, "");
            $dsctos_por_seguro_de_vida_ccaf  = rellenar($_POST['dsctos_por_seguro_de_vida_ccaf'][$i], 8, "");
            $otros_descuentos_ccaf  = rellenar($_POST['otros_descuentos_ccaf'][$i], 8, "");
            $cotizac_a_ccaf_no_afiliados_isapres = rellenar($_POST['cotizac_a_ccaf_no_afiliados_isapres'][$i], 8, "");
            $descuento_cargas_familiares_ccaf = rellenar($_POST['descuento_cargas_familiares_ccaf'][$i], 8, "");
            $otros_descuentos_ccaf_1_futuro  = rellenar($_POST['otros_descuentos_ccaf_1_futuro'][$i], 8, "");
            $otros_descuentos_ccaf_2_futuro  = rellenar($_POST['otros_descuentos_ccaf_2_futuro'][$i], 8, "");
            $bonos_gobierno_futuro  = rellenar($_POST['bonos_gobierno_futuro'][$i], 8, "");
            $codigo_de_sucursal_futuro   = rellenar($_POST['codigo_de_sucursal_futuro'][$i],20, "");
            $codigo_mutualidad  = rellenar($_POST['codigo_mutualidad'][$i], 2, "");
            $renta_imponible_mutual  = rellenar($_POST['renta_imponible_mutual'][$i], 8, "");
            $cotizacion_mutual  = rellenar($_POST['cotizacion_mutual'][$i], 8, "");
            $sucursal_para_pago_mutual  = rellenar($_POST['sucursal_para_pago_mutual'][$i], 3, "");
            $renta_imp_seguro_cesantia  = rellenar($_POST['renta_imp_seguro_cesantia'][$i], 8, "");
            $aporte_trabajador_seguro_cesantia  = rellenar($_POST['aporte_trabajador_seguro_cesantia'][$i], 8, "");
            $aporte_empleador_seguro_cesantia = rellenar($_POST['aporte_empleador_seguro_cesantia'][$i], 8, "");
            $rut_pagadora_subsidio   = rellenar($_POST['rut_pagadora_subsidio'][$i],11, "");
            $dv_pagadora_subsidio = rellenar($_POST['dv_pagadora_subsidio'][$i], 1, "");
            $ccosots_suc_agencia_obra_region  = rellenar($_POST['ccosots_suc_agencia_obra_region'][$i],20, "");
