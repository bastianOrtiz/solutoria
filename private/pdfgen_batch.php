...
if(!$_GET['ids']){
    redirect(BASE_URL.'/liquidacion/batch',null,true,'No ha seleccionado liquidaciones');
    exit();
}
...
foreach($liquidaciones_ids as $liquidacion_id){
    ...
    $apvTrabajador = apvTrabajador($trabajador_id);
    if( $apvTrabajador ){
        foreach( $apvTrabajador as $apv ){
            if( $apv['tipo'] == 'A' ){
                    ...
                    <table style="width:100%" class="subtable">
                        <tr>
                            <td style="padding:0; text-align: left; width:85%">
                                ...
                                <small>Total Tipo A: </small>';
                                $content .= '$ '. number_format($total_apv_A,0,',','.');

                                $content .= '<br><small>Total Tipo B: </small>';
                                $content .= '$ '. number_format($total_apv_B,0,',','.');
